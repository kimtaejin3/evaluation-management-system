...
- [ ] [E] 위원장만 대상별 화면(`/evaluate/[sessionId]/chair/[subjectId]`)에서 타 위원 점수·항목별 의견을 열람한다(아니면 403→리다이렉트, getChairSubjectData)
- [ ] [U] 위원 입력 상태 판정: 전 채점 단위 입력=complete, 일부=partial, 없음=none (chair-subject.chairEvalState)
- [ ] [E] 종합의견은 위원장만 대상별로 1건 저장한다(빈 값은 삭제), 저장분은 평가의견서·집계결과에 표시된다 (saveChairOpinion)
- [ ] [U] 종합의견 저장은 chairId≠userId면 "위원장만 작성할 수 있습니다." 오류 (saveChairOpinion)
- [ ] [E] 진행중이 아니거나 배정이 유효하지 않으면 종합의견 저장이 거부된다 (saveChairOpinion)
- [ ] [E] 의견서가 제출(SUBMITTED)/승인(APPROVED)되면 위원장도 종합의견을 수정·삭제할 수 없다(서버 거부 + 화면 읽기 전용)
...
- [ ] [E] 평가표 양식: 메타헤더(지역/과제유형/과제명/기관/책임자) + 3단 표(배점·평점) + 합계 자동합 + 서명란 (종합의견은 위원장 전용이라 평가표에 없다)
