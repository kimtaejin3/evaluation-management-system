...
async function main() {
  ...
  const coA = await prisma.company.upsert({ where: { name: 'E2E 대상 A' }, update: {}, create: { name: 'E2E 대상 A' } })
  const coB = await prisma.company.upsert({ where: { name: 'E2E 대상 B' }, update: {}, create: { name: 'E2E 대상 B' } })
  const subA = await prisma.subject.create({ data: { sessionId: session.id, companyId: coA.id, name: '대상 A', order: 0 } })
  const subB = await prisma.subject.create({ data: { sessionId: session.id, companyId: coB.id, name: '대상 B', order: 1 } })
