...
export type SubjectField = 'name' | 'businessNo' | 'region' | 'leadResearcher' | 'description'
...
export const SUBJECT_FIELDS: SubjectFieldDef[] = [
  ...
  { field: 'region', label: '지역', required: false },
  { field: 'leadResearcher', label: '연구책임자', required: false },
  { field: 'description', label: '설명', required: false },
]

const SYNONYMS: Record<SubjectField, string[]> = {
  name: ['기업명', '업체명', '회사명', '상호명', '상호', '대상명', '평가대상명', '평가대상', '기관명', '명칭', '기업', '업체', '회사'],
  businessNo: ['사업자등록번호', '사업자번호', '사업자', '등록번호', 'businessno', 'bizno'],
  region: ['지역', '소재지', '지역명', '주소', 'region', 'area'],
  leadResearcher: ['연구책임자', '연구책임', '책임자', '책임연구원', '연구책임연구원', '대표연구자', 'pi'],
  description: ['설명', '사업개요', '개요', '비고', '내용'],
}
...
export function buildSubjects(
  ...
  const regionCol = mapping.findIndex((f) => f === 'region')
  const leadCol = mapping.findIndex((f) => f === 'leadResearcher')
  ...
  dataRows.forEach((r) => {
    ...
    rows.push({
      name,
      businessNo: val(r, bizCol),
      region: val(r, regionCol),
      leadResearcher: val(r, leadCol),
      description: val(r, descCol),
    })
  })
