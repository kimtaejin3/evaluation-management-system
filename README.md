...
- **관리자(admin / admin1234)**: 회차 생성 → 평가 항목(정량/정성·배점·가중치) → 평가 대상 + **심사 서류 업로드** → 평가위원 배정 → `평가 시작` → **진행 현황 모니터링** → `마감·잠금` → 결과표·산출 근거 조회·CSV·인쇄
- **평가위원**: 관리자가 발급한 계정으로 로그인 → 배정된 진행중 회차의 대상별 **심사 서류 열람** + 점수 입력·제출

## 주요 기능

- 사이드바·대시보드(KPI) 기반 서비스형 UI (관리자/평가위원 포털)
- 평가 항목: 정량/정성·배점·가중치, **템플릿 저장·불러오기**, **회차 복사**
- 평가 대상별 **심사 서류 업로드·열람** (인증 보호된 `/api/documents/[id]` 서빙)
- **진행 현황 모니터링**: 위원×대상 완료 그리드 + 전체 진행률
- 집계 결과(순위) + **위원별 점수·산출 근거표**, CSV·인쇄
- 회차 상태 흐름(초안→진행중→마감) 및 마감 시 데이터 잠금
...
npx tsx scripts/demo-seed.ts   # 데모 데이터(회차·항목·대상·위원·점수) 생성
```
