...
export default function EvaluateHomeClient({
  submitted,
}: {
  submitted?: string;
}) {
    ...
    <div className="mx-auto max-w-5xl space-y-6 px-6 py-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          {name ? `${name} 위원님, ` : ""}대상을 선택해 평가를 진행하세요.
        </p>
      </div>
      ...
      {!error && sessions === null && (
        <SkeletonCardGrid count={2} lines={4} cols="" />
      )}
      ...
      {sessions?.map((s) => {
        const pct =
          s.totalSubjects > 0
            ? Math.round((s.doneSubjects / s.totalSubjects) * 100)
            : 0;
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-[var(--gov-navy)] px-5 py-3 text-white">
              ...
              <div className="flex items-center gap-3 text-sm">
                <span className="text-slate-200">
                  {s.doneSubjects}/{s.totalSubjects} 완료
                </span>
