...
async function AssignmentList({ userId, userName }: { userId: string; userName: string }) {
  ...
  for (const s of myScores) {
    ...
    rowsBySubject
      .get(s.subjectId)!
      .push({ criterionId: s.criterionId, value: s.value });
  }
  ...
  return (
    <>
      ...
      {assignments.map((a) => {
        ...
        const weights = a.session.criteria.map((c) => ({
          id: c.id,
          weight: c.weight,
        }));
        const doneSubjects = a.session.subjects.filter(
          (sub) =>
            total > 0 && (rowsBySubject.get(sub.id)?.length ?? 0) >= total,
        ).length;
        const pct =
          a.session.subjects.length > 0
            ? Math.round((doneSubjects / a.session.subjects.length) * 100)
            : 0;
        const deadline = a.session.eventDate
          ? new Date(a.session.eventDate).toLocaleString("ko-KR", {
              month: "long",
              day: "numeric",
              hour: "2-digit",
              minute: "2-digit",
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-[var(--gov-navy)] px-5 py-3 text-white">
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-slate-200">
                  {doneSubjects}/{a.session.subjects.length} 완료
                </span>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              {a.session.subjects.map((sub, i) => {
                ...
                const score = complete
                  ? computeWeightedScore(rows, weights)
                  : null;
                // 이 심사 전용 + 공통 자료
                const docs = sub.company.documents.filter(
                  (d) => d.sessionId === a.session.id || d.sessionId === null,
                );
                return (
                  <div
                    key={sub.id}
                    className="border-b border-slate-100 px-5 py-4 transition last:border-0 hover:bg-slate-50"
                  >
                    <div className="flex items-center gap-4">
                      <span className="w-6 shrink-0 text-sm font-semibold text-slate-400 tabular-nums">
