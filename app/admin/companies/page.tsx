  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">평가 대상 기업과 자료를 전역으로 등록합니다. 자료는 여러 회차에서 공유됩니다.</p>
      </div>

      {/* 신규 기업 등록 (상단) */}
      <form action={createCompany} className="grid grid-cols-3 gap-3 rounded-lg border border-slate-200 bg-white p-4">
        ...
        {companies.map((c) => (
          <div key={c.id} className="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4 border-t border-slate-100 pt-4">
              ...
              <form action={uploadCompanyDocument.bind(null, c.id)} className="mt-3 flex items-center gap-2">
                <input type="file" name="file" multiple required className="block w-full text-sm text-slate-600 file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-sm file:text-slate-700 hover:file:bg-slate-200" />
