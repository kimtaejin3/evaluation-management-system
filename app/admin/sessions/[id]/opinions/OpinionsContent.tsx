...
// 분과 평가 의견서 본문 — 분과 페이지와 사업 평가의견서(분과 선택 임베드)가 공유한다.
export default async function OpinionsContent({ id }: { id: string }) {
  ...
  // 위원장 지정·검토 완료는 담당자의 일이므로 관리자에게는 이 안내를 띄우지 않는다.
  const blockers =
    !isMaster && !locked && (os === "DRAFT" || os === "REJECTED") ? await opinionReviewBlockers(id) : [];
