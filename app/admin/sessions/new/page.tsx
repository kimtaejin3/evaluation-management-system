...
export default function NewSessionPage() {
  return (
    <div className="max-w-2xl">
      <Link href="/admin/sessions" className="text-sm text-slate-400 hover:text-slate-600">← 회차 목록</Link>
      ...
      <form action={createSession} className="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="space-y-4 p-5">
          ...
          <div>
            ...
            <textarea name="description" rows={2} className={`${inputCls} resize-none`} placeholder="회차에 대한 간단한 설명(선택)" />
          </div>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 bg-slate-50/60 px-5 py-3">
          ...
          <button className="rounded-md bg-indigo-600 px-5 py-2 text-sm font-medium text-white transition hover:bg-indigo-700">회차 생성</button>
        </div>
      </form>
    </div>
  )
