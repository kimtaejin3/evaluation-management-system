              ...
              <div className="max-h-56 overflow-auto rounded-xl border border-slate-200 bg-white">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-slate-50 text-left text-slate-500">
                    <tr className="border-b border-slate-200">
                      {mapping.includes("section") && <th className="whitespace-nowrap px-3 py-2 font-medium">항목</th>}
                      <th className="px-3 py-2 font-medium">세부항목</th>
                      {mapping.includes("description") && <th className="px-3 py-2 font-medium">평가 관점(설명)</th>}
                      <th className="whitespace-nowrap px-3 py-2 font-medium">방식</th>
                  ...
                  <tbody>
                    {preview.rows.slice(0, 100).map((r, i) => (
                      <tr key={i} className="border-b border-slate-100 align-top">
                        {mapping.includes("section") && (
                          <td className="whitespace-nowrap px-3 py-2 text-slate-500">{r.section ?? "—"}</td>
                        )}
                        <td className="px-3 py-2 text-slate-800">
                          {r.name}
                          {r.type === "QUALITATIVE" && r.gradeOptions && (
                            <span className="ml-2 text-xs text-slate-400">
                              {r.gradeOptions.map((g) => `${g.label} ${g.points}`).join(" / ")}
                            </span>
                          )}
                        </td>
                        {mapping.includes("description") && (
                          <td className="min-w-56 whitespace-pre-line px-3 py-2 text-slate-500">
                            {r.description ?? "—"}
                          </td>
                        )}
                        <td className="whitespace-nowrap px-3 py-2 text-slate-500">
                          {r.type === "QUALITATIVE" ? "정성" : "정량"}
                        </td>
