...
export default function ProjectTabNav() {
  ...
  return (
    <div className="flex items-center justify-end gap-4 px-8 pt-3 print:hidden">
      {prev && (
        <Link href={`/admin/projects/${id}${prev.suffix}`} className={`inline-flex items-center gap-1 ${link}`}>
      ...
      {next && (
        <Link href={`/admin/projects/${id}${next.suffix}`} className={`inline-flex items-center gap-1 ${link}`}>
          {next.label}
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4 shrink-0" aria-hidden>
