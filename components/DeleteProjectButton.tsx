      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-slate-900">과제 삭제</h3>
            <p className="mt-2 text-sm text-slate-600">
              <span className="font-medium text-slate-800">{projectName}</span> 과제를 삭제합니다.
            </p>
            {sessionCount > 0 ? (
              <p className="mt-1.5 text-xs text-slate-500">
                소속 분과 <span className="font-medium text-slate-700">{sessionCount}개</span>는 삭제되지 않고 <span className="font-medium">‘미분류’</span> 상태로 남습니다(분과의 평가 항목·대상·점수는 보존). 다만 과제-분과 연결과 간사 배정 정보는 사라집니다.
              </p>
            ) : (
              <p className="mt-1.5 text-xs text-slate-500">이 과제에는 소속 분과가 없습니다.</p>
            )}
