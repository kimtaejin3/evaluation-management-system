...
import {
  autoDetectEvalAccountMapping,
  evalAccountLooksLikeHeader,
  buildEvalAccounts,
  EVAL_ACCOUNT_FIELDS,
  type EvalAccountColumnMapping,
  type EvalAccountField,
} from "@/lib/evaluator-account-import";
import { parseTsv } from "@/lib/kpass-import";
import { commitEvalAccountImport, type EvalAccountImportResult } from "@/app/admin/actions";
...
export default function EvaluatorAccountImportForm({ onDone }: { onDone: () => void }) {
  ...
  const [override, setOverride] = useState<Record<number, EvalAccountField | null>>({});
  const [result, setResult] = useState<EvalAccountImportResult | null>(null);
  ...
  useEffect(() => {
    ...
    setHasHeader(grid.length > 0 ? evalAccountLooksLikeHeader(grid[0]) : true);
  }, [grid]);

  const headerRow = hasHeader ? grid[0] ?? [] : [];
  const auto = useMemo<EvalAccountColumnMapping>(() => {
    const padded = Array.from({ length: colCount }, (_, i) => headerRow[i] ?? "");
    return autoDetectEvalAccountMapping(padded);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [colCount, headerRow.join(""), hasHeader]);

  const mapping: EvalAccountColumnMapping = useMemo(
    () => Array.from({ length: colCount }, (_, i) => (i in override ? override[i] : auto[i] ?? null)),
    [colCount, override, auto],
  );

  const preview = useMemo(() => buildEvalAccounts(grid, mapping, { hasHeader }), [grid, mapping, hasHeader]);
  ...
  const setCol = (i: number, v: string) =>
    setOverride((o) => ({ ...o, [i]: v === "" ? null : (v as EvalAccountField) }));

  const submit = () => {
    setResult(null);
    startTransition(async () => {
      const res = await commitEvalAccountImport({ grid, mapping, hasHeader });
      setResult(res);
  ...
  return (
    <div className="space-y-5">
      <section className="space-y-2">
        <label className="text-sm font-semibold text-slate-700">1. 평가위원 명단 — 엑셀 파일 업로드 또는 붙여넣기</label>
