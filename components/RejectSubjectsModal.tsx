...
import { rejectSubjects } from '@/app/admin/sessions/actions'

// 평가 대상 전체 반려 — 사유 입력 모달(마스터 전용)
export default function RejectSubjectsModal({ sessionId }: { sessionId: string }) {
  ...
  const submit = () => {
    ...
    if (!trimmed) return
    start(async () => {
      await rejectSubjects(sessionId, trimmed)
      setOpen(false)
      setReason('')
      router.refresh()
    })
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="text-base font-semibold text-slate-900">평가 대상 반려</h3>
              <button type="button" onClick={() => setOpen(false)} className="text-slate-400 transition hover:text-slate-600">
                ✕
              </button>
            </div>
            <p className="mt-2 text-xs text-slate-400">
              이 분과의 모든 평가 대상이 반려 처리되며, 담당 간사에게 사유가 표시됩니다.
            </p>
            <textarea
              value={reason}
