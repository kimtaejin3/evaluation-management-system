...
describe('CriteriaEditor 추가 UX', () => {
  it('평가항목 추가 시 입력창이 즉시 닫히고 항목이 바로 나타난다(서버 응답 전)', async () => {
    ...
    render(<CriteriaEditor sessionId="s1" groups={[]} />)
  ...
  it('세부항목 추가 시 입력창이 즉시 닫히고 세부항목이 바로 나타난다', async () => {
    const user = userEvent.setup()
    render(<CriteriaEditor sessionId="s1" groups={[{ id: 'g1', name: '사업계획', maxScore: 10, subitems: [] }]} />)
    await user.click(screen.getByRole('button', { name: '+ 세부항목' }))
    await user.type(screen.getByPlaceholderText('세부항목명'), '타당성')
    await user.click(screen.getByRole('button', { name: '추가' }))
    expect(screen.getByText('타당성')).toBeInTheDocument()
    expect(screen.queryByPlaceholderText('세부항목명')).toBeNull()
  })
})
