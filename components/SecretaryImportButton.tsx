...
import SecretaryImportForm from './SecretaryImportForm'

// 담당자 엑셀 일괄 등록 트리거 — 버튼 + 모달. 담당자 관리(전역, 마스터 전용)에서 사용.
export default function SecretaryImportButton() {
        ...
        >
          <div className="my-8 w-full max-w-3xl rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-base font-semibold text-slate-800">담당자 엑셀 업로드</h3>
                <p className="mt-0.5 text-xs text-slate-400">엑셀 파일을 업로드하거나 복사·붙여넣어 담당자를 일괄 등록합니다.</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="text-slate-400 hover:text-slate-600">
                ✕
              </button>
            </div>
            <SecretaryImportForm onDone={() => setOpen(false)} />
          </div>
        </div>
      )}
    </>
