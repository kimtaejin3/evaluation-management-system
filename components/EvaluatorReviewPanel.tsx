...
export default function EvaluatorReviewPanel({
  ...
  const labelCls: Record<EvaluatorStatus, string> = {
...
function RejectButton({
  ...
  const submit = () => {
    ...
    if (!trimmed) return
    run(async () => {
      await rejectEvaluators(sessionId, trimmed)
      setOpen(false)
      setReason('')
    })
  }
